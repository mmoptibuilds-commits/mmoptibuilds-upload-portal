...
import { CustodyStrip, EmptyState, Metric, Status } from "@/components/ui";
...
export default async function AdminPage() {
  ...
  return <AppShell user={user}>
    <header className="page-header"><div><CustodyStrip route="ADMIN">OPS:SUMMARY ▦ QUEUES:VISIBLE ▦ SECRETS:HIDDEN</CustodyStrip><span className="eyebrow">ADMIN CONTROL ROOM</span><h1>Transfer systems</h1><p>Users, delivery state, and private storage health in one controlled surface.</p></div><Link className="button button-primary" href="/admin/users">Manage users</Link></header>
    <section className="metric-grid"><Metric label="Total users" value={s.users} detail={`${s.activeUsers} enabled`} /><Metric label="Upload batches" value={s.batches} detail={`${s.failed} need attention`} /><Metric label="Recorded delivery" value={`${(s.bytes / 1024 / 1024 / 1024).toFixed(1)} GB`} detail="Metadata only" /><Metric label="Storage state" value={config.storage ? "Ready" : "Missing"} detail={config.storage ? "Private bucket connected" : "Check diagnostics"} /></section>
