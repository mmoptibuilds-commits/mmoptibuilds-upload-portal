...
export function RippleButton({ children, className = "", ...props }: PropsWithChildren<ButtonHTMLAttributes<HTMLButtonElement>>) {
  const ref = useRef<HTMLButtonElement>(null); const reduce = useReducedMotion();
  return <motion.button ref={ref} className={`button ${className}`} whileHover={reduce ? undefined : { scale: 1.025, y: -1 }} whileTap={{ scale: 0.97 }} onPointerDown={(event) => {
